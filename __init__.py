...
class AlbumArtistWebsite:
	...
	def add_artist_website(
		...
		for artist_id in album_artist_ids:
			if artist_id in self.website_cache:
				if self.website_cache[artist_id]:
					self.add_websites_to_track(track, self.website_cache[artist_id])
			else:
				self.website_add_track(album, track, artist_id)

	def website_add_track(self, album: Album, track: Track, artist_id: str):
		"""Queue a track for website fetching and make webservice request if needed."""
		if self.website_queue.append(artist_id, (track, album)):
			task_id = f"artist_website_{artist_id}"
			self.api.add_album_task(
				album,
				task_id,
				f"Fetching artist website for {artist_id}",
				request_factory=lambda: self.api.mb_api._get_by_id(
					"artist",
					artist_id,
					partial(self.website_process, artist_id),
					inc=["url-rels"],
					priority=True,
					important=False,
	...
	def website_process(self, artist_id: str, response, reply, error):
		"""Process the artist website response from MusicBrainz."""
		if error:
			self.api.logger.error(
				"Network error retrieving artist record for %s: %s", artist_id, error
			)
			tuples = self.website_queue.remove(artist_id)
			if tuples:
				for track, album in tuples:
					self.api.complete_album_task(album, f"artist_website_{artist_id}")
			return

		urls = self.artist_process_metadata(artist_id, response)
		self.website_cache[artist_id] = urls
		tuples = self.website_queue.remove(artist_id)
	...
	def artist_process_metadata(self, artist_id: str, response):
		"""Extract official homepage URLs from artist metadata response."""
		self.api.logger.debug(
			"Processing artist record for official website urls: %s", artist_id
		)
		relations = self.artist_get_relations(response)

		if not relations:
			self.api.logger.info("Artist %s does not have any associated urls", artist_id)
			return []
			...
			if "type" in relation.attribs and relation.type == "official homepage":
				if "target" in relation.children and len(relation.target) > 0:
					if "ended" not in relation.children or relation.ended[0].text != "true":
						...
						self.api.logger.debug(
							"Artist url has ended: %s", relation.target[0].text
						)
				else:
					self.api.logger.debug("No url in relation: %s", relation)

		if urls:
			self.api.logger.info("Artist %s official homepages: %s", artist_id, urls)
		else:
			self.api.logger.info(
				"Artist %s does not have any official website urls", artist_id
			)
	...
	def artist_get_relations(self, response):
		...
		if "metadata" in response.children and len(response.metadata) > 0:
			if (
				"artist" in response.metadata[0].children
				and len(response.metadata[0].artist) > 0
			):
				if (
					"relation_list" in response.metadata[0].artist[0].children
					and len(response.metadata[0].artist[0].relation_list) > 0
				):
					if (
						"relation"
						in response.metadata[0].artist[0].relation_list[0].children
					):
						relations = (
							response.metadata[0].artist[0].relation_list[0].relation
						)
						self.api.logger.debug(
							"artist_get_relations returning: %s", relations
						)
						return relations
					else:
						self.api.logger.debug(
							"artist_get_relations - no relation in relation_list"
						)
				else:
